...
//DONE: CxHandle ()
/*explicit*/
CxHandle::CxHandle(HANDLE hHandle) :
    m_hHandle(hHandle)
{
    /*DEBUG*/// n/a
}
//---------------------------------------------------------------------------
//DONE: CxHandle ()
CxHandle::CxHandle(const CxHandle &hHandle) :
...
CxHandle::operator = (const CxHandle &hHandle) {
    ...
    if (this != &hHandle) {
        ...
        m_hHandle = hHandle.hDuplicate(hGetCurrentProcess(), DUPLICATE_SAME_ACCESS, FALSE, DUPLICATE_SAME_ACCESS);
        /*DEBUG*/xASSERT_RET(NULL != m_hHandle, *this);
    }
...
CxHandle::operator HANDLE() const {
    /*DEBUG*///???
...
CxHandle::bIsValid() const {
    ...
    */
    ...
    BOOL bCond7 = (NULL                            != m_hHandle);    //NULL
...
//DONE: bAttach ()
BOOL
CxHandle::bAttach(HANDLE hHandle) {
...
ULONG
CxHandle::ulGetInformation() const {
...
BOOL
CxHandle::bSetInformation(ULONG ulMask, ULONG ulFlags) {
    /*DEBUG*/xASSERT_RET(FALSE != bIsValid(), FALSE);
    /*DEBUG*///ulMask  - n/a
    /*DEBUG*///ulFlags - ????
...
BOOL
CxHandle::bIsFlagProtectFromClose() const {
    /*DEBUG*/xASSERT_RET(FALSE != bIsValid(), FALSE);
...
BOOL
CxHandle::bSetFlagProtectFromClose(BOOL bFlagProtectFromClose) {
    /*DEBUG*/xASSERT_RET(FALSE != bIsValid(), FALSE);
...
HANDLE
CxHandle::hDuplicate(HANDLE hTargetProcess, ULONG ulDesiredAccess, BOOL bInheritHandle/* = FALSE*/, ULONG ulOptions/* = 0*/) const {
...
//DONE: static
const HANDLE
CxHandle::_ms_chCurrProcessHandle = (HANDLE) - 1;
//---------------------------------------------------------------------------
//DONE: hGetCurrent (Retrieves a pseudo handle for the current process)
/*static*/
HANDLE
CxHandle::hGetCurrentProcess() {
    /*DEBUG*///n/a

    HANDLE hRes = NULL/*hFailValueT*/;

    hRes = ::GetCurrentProcess();
    /*DEBUG*/xASSERT_RET(_ms_chCurrProcessHandle == hRes, NULL/*NULL*/);
...
/*static*/
BOOL
CxHandle::bIsValid(HANDLE hHandle) {
    /*DEBUG*/// n/a

    return /*NULL*/NULL != hHandle;
}
//---------------------------------------------------------------------------

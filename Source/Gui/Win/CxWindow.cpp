                       ...
                       INT iLeft, INT iTop, INT iWidth, INT iHeight, ULONG ulStyle, ULONG ulStyleEx, LPVOID lpParam)
{
    ...
    hWnd = ::CreateWindowEx(_m_ulStyleEx | ulStyleEx,
                            ...
                            hParent,
                            (tString::npos != csClassName.find(xCXWINDOW_CONTROL_CLASS)) ? static_cast<HMENU>(NULL) : reinterpret_cast<HMENU>(iID),    /*���� - NULL, ������� - iID*/
                            CxApplication::hGetInstance(),
                            lpParam);
    /*DEBUG*/xASSERT_RET(NULL != hWnd, FALSE);
    /*DEBUG*/xCHECK_DO(FALSE == _m_bIsControl, xASSERT_RET(::GetDlgCtrlID(hWnd) == 0,   FALSE));    //����
    /*DEBUG*/xCHECK_DO(TRUE  == _m_bIsControl, xASSERT_RET(::GetDlgCtrlID(hWnd) == iID, FALSE));    //�������

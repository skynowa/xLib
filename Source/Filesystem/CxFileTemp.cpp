...
)
{
    ...
    _m_sFilePath = CxPath::sSlashAppend(csDirPath) + CxPath::sGetFullName(csFilePath) + csFileNameTemplate;
        ...
        TCHAR *pszFile = xTMKSTEMP(&_m_sFilePath.at(0));
        /*DEBUG*/xASSERT_RET(NULL != pszFile, FALSE);

        _pfStdFile = xTFOPEN(pszFile, CxFile::_sGetOpenMode(CxFile::omBinCreateReadWrite).c_str());
        ...
        _pfStdFile = xTFOPEN(&_m_sFilePath.at(0), CxFile::_sGetOpenMode(CxFile::omBinCreateReadWrite).c_str());
        /*DEBUG*/xASSERT_RET(NULL != _pfStdFile, FALSE);
    #endif

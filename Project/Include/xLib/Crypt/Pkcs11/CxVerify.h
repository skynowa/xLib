...
public:
    ...
    void_t               init(CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey);
        ...
        ///< and plaintext cannot cannot be recovered from the signature (e.g. DSA)
    void_t               make(CK_BYTE_PTR pData, CK_ULONG ulDataLen, CK_BYTE_PTR pSignature,
                             CK_ULONG ulSignatureLen);
        ///< verifies a signature in a single-part operation,  where the signature is an appendix
        ///< to the data, and plaintext cannot be recovered from the signature
    ...
    void_t               recoverInit(CK_MECHANISM_PTR pMechanism, CK_OBJECT_HANDLE hKey);
        ...
        ///< from the signature
    void_t               recover(CK_BYTE_PTR pSignature, CK_ULONG ulSignatureLen, CK_BYTE_PTR pData,
    ...
    void_t               update(CK_BYTE_PTR pPart, CK_ULONG ulPartLen);
        ///< continues a multiple-part verification operation, where the signature is an appendix
        ///< to the data, and plaintext cannot be recovered from the signature

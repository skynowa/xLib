...
//---------------------------------------------------------------------------
#define _xTEST_MSG_EQ(report_type, val1, val2, msg)      { if ( !((val1) == (val2)) ) { ulong_t ulLastError = CxLastError::ulGet(); CxErrorReport rpReport(report_type, (val1),        (val2),         xT(#val1),   xT(#val2), xT("=="), ulLastError, xFILE, xLINE, xFUNCTION, xDATE, xTIME, CxStackTrace().sGet(), (msg));  CxDebugger().bReportMake(rpReport); } }
#define _xTEST_MSG_DIFF(report_type, val1, val2, msg)    { if ( !((val1) != (val2)) ) { ulong_t ulLastError = CxLastError::ulGet(); CxErrorReport rpReport(report_type, (val1),        (val2),         xT(#val1),   xT(#val2), xT("!="), ulLastError, xFILE, xLINE, xFUNCTION, xDATE, xTIME, CxStackTrace().sGet(), (msg));  CxDebugger().bReportMake(rpReport); } }
#define _xTEST_MSG_LESS(report_type, val1, val2, msg)    { if ( !((val1) <  (val2)) ) { ulong_t ulLastError = CxLastError::ulGet(); CxErrorReport rpReport(report_type, (val1),        (val2),         xT(#val1),   xT(#val2), xT("<"),  ulLastError, xFILE, xLINE, xFUNCTION, xDATE, xTIME, CxStackTrace().sGet(), (msg));  CxDebugger().bReportMake(rpReport); } }
#define _xTEST_MSG_GR(report_type, val1, val2, msg)      { if ( !((val1) >  (val2)) ) { ulong_t ulLastError = CxLastError::ulGet(); CxErrorReport rpReport(report_type, (val1),        (val2),         xT(#val1),   xT(#val2), xT(">"),  ulLastError, xFILE, xLINE, xFUNCTION, xDATE, xTIME, CxStackTrace().sGet(), (msg));  CxDebugger().bReportMake(rpReport); } }
#define _xTEST_MSG_LESS_EQ(report_type, val1, val2, msg) { if ( !((val1) <= (val2)) ) { ulong_t ulLastError = CxLastError::ulGet(); CxErrorReport rpReport(report_type, (val1),        (val2),         xT(#val1),   xT(#val2), xT("<="), ulLastError, xFILE, xLINE, xFUNCTION, xDATE, xTIME, CxStackTrace().sGet(), (msg));  CxDebugger().bReportMake(rpReport); } }
#define _xTEST_MSG_GR_EQ(report_type, val1, val2, msg)   { if ( !((val1) >= (val2)) ) { ulong_t ulLastError = CxLastError::ulGet(); CxErrorReport rpReport(report_type, (val1),        (val2),         xT(#val1),   xT(#val2), xT(">="), ulLastError, xFILE, xLINE, xFUNCTION, xDATE, xTIME, CxStackTrace().sGet(), (msg));  CxDebugger().bReportMake(rpReport); } }
#define _xTEST_MSG_PTR(report_type, ptr, msg)            { if ( NULL     == (ptr)   ) { ulong_t ulLastError = CxLastError::ulGet(); CxErrorReport rpReport(report_type, (intptr_t)ptr, (intptr_t)NULL, xT("NULL"),  xT(#ptr),  xT("!="), ulLastError, xFILE, xLINE, xFUNCTION, xDATE, xTIME, CxStackTrace().sGet(), (msg));  CxDebugger().bReportMake(rpReport); } }
#define _xTEST_MSG_PTR_FAIL(report_type, ptr, msg)       { if ( NULL     != (ptr)   ) { ulong_t ulLastError = CxLastError::ulGet(); CxErrorReport rpReport(report_type, (intptr_t)ptr, (intptr_t)NULL, xT("NULL"),  xT(#ptr),  xT("=="), ulLastError, xFILE, xLINE, xFUNCTION, xDATE, xTIME, CxStackTrace().sGet(), (msg));  CxDebugger().bReportMake(rpReport); } }
#define _xTEST_MSG_FAIL(report_type, msg)                { if ( true                ) { ulong_t ulLastError = CxLastError::ulGet(); CxErrorReport rpReport(report_type, xT(""),        xT(""),         xT("false"), xT(""),    xT(""),   ulLastError, xFILE, xLINE, xFUNCTION, xDATE, xTIME, CxStackTrace().sGet(), (msg));  CxDebugger().bReportMake(rpReport); } }
#define _xTESTMSG_(report_type, expr, msg)               { if ( !(expr)             ) { ulong_t ulLastError = CxLastError::ulGet(); CxErrorReport rpReport(report_type, (true),        (expr),         xT("false"), xT(#expr), xT("=="), ulLastError, xFILE, xLINE, xFUNCTION, xDATE, xTIME, CxStackTrace().sGet(), (msg));  CxDebugger().bReportMake(rpReport); } }
...
#define xSTD_VERIFY(expr)                                { \
                                                            if ( !(expr) )  { \
                                                                const std::tstring_t sLastError = CxLastError::sGet(); \
                                                                CxTracer::vWrite(xT("\n--------------------------------- xSTD_VERIFY ----------------------------------\n") \
                                                                                 xT("  Expression: %s\n") \
                                                                                 xT("  File:       %s\n") \
                                                                                 xT("  Function:   %s\n") \

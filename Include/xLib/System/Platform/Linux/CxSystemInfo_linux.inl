...
CxSystemInfo::_cpuVendor_impl() const
{
    ...
    std::tstring_t sRv = CxPath::procValue(xT("/proc/cpuinfo"), xT("vendor_id"));
...
CxSystemInfo::_cpuModel_impl() const
{
    ...
    std::tstring_t sRv = CxPath::procValue(xT("/proc/cpuinfo"), xT("model name"));
    xTEST_EQ(sRv.empty(), false);
...
CxSystemInfo::_cpuSpeed_impl() const
{
    ...
    ulong_t ulRv = CxUtils::roundIntT<ulong_t>( cpuSpeedMHz );
...
CxSystemInfo::_ramUsage_impl() const
{
    ...
    ulong_t ulRv = static_cast<ulong_t>( CxUtils::safeDivT(usage * 100.0, info.totalram) );

...
{
    const char *pcszRv = ::getenv(xT2A(_prefix_name).c_str());
    xTEST_NA(pcszRv);
...
) const
{
    ...
    int_t iRv = ::setenv(xT2A(_prefix_name).c_str(), xT2A(a_value).c_str(), isReplaced);
    xTEST_DIFF(iRv, - 1);
}
//-------------------------------------------------------------------------------------------------
void_t
...
{
#if   xENV_LINUX
    int_t iRv = ::unsetenv(xT2A(_prefix_name).c_str());
    xTEST_DIFF(iRv, - 1);
#elif xENV_BSD
    (void_t)::unsetenv(xT2A(_prefix_name).c_str());
#elif xENV_APPLE
    int_t iRv = ::unsetenv(xT2A(_prefix_name).c_str());
    xTEST_DIFF(iRv, - 1);
#endif

...
)
{
    ...
    a_stdFile = std::xTFOPEN(file, File::_openMode(File::omBinCreateReadWrite).c_str());
    xTEST_PTR(a_stdFile);
#else
    _filePath.resize(_filePath.size() + 1);
    ...
    a_stdFile = std::xTFOPEN(_filePath.c_str(), File::_openMode(File::omBinCreateReadWrite).c_str());
    xTEST_PTR(a_stdFile);
#endif
}
//-------------------------------------------------------------------------------------------------

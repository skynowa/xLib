...
#define xLib_Common_Win_CxPerfCountersH
//---------------------------------------------------------------------------
...
protected:
    ...
    GetCounterValue(PPERF_OBJECT_TYPE pPerfObj, DWORD dwCounterIndex, LPCTSTR pInstanceName) {
        ...
        if ( pPerfObj->NumInstances == PERF_NO_INSTANCES ) {
            ...
            /*_bstr_t*/CComBSTR bstrInstance;   ////CComBSTR  bstrInstance;
            /*_bstr_t*/CComBSTR bstrInputInstance = pInstanceName;
            for( int k=0; k < pPerfObj->NumInstances; k++ )    {
                ...
                if ((String(bstrInstance) != String(bstrInputInstance)))
                {
